...
window.OmnitoneDemoPlayer = (function() {
  ...
  // Change: The above line was removed because the "Gallery" button is no longer needed or has been replaced by another UI element.
  // This button was originally added to the interface to allow users to open a gallery, but it's no longer part of the user interface.
  */
  var _buttonPause;
  var _buttonRewind;
  ...
  function getVideoSourceData() {
    ...
    var contentId = "fuerzaimprevista";
    var contentData;
    if (!contentId) {
      VRSamplesUtil.makeToast(_infoMessage, 1);
      VRSamplesUtil.addError(
    ...
    if (_browserInfo.platform === 'Android') {
      ...
      */
      ...
      // Change: The above line was commented out and replaced with the line below to switch from the 720p video source to the 1080p video source.
      // The change allows the player to use the higher resolution video (mp4_1080p) instead of the lower resolution (mp4_720p), likely for a better viewing experience.
  ...
  function greetAudience() {
    ...
    // Change: The above line was removed because the instructional message is no longer necessary or has been replaced by other UI elements. 
    // This message was originally displayed to guide the user about dragging the screen for spatial audio and pressing "Play" to start the video.
    */
